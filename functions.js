...
function toTeenCode6(text) {
  ...
  for (i in textArr) {
    ...
    var isMatch = null;
    isMatch = word.match(
      /^([a|à|á|ạ|ả|ã|â|ầ|ấ|ậ|ẩ|ẫ|ă|ằ|ắ|ặ|ẳ|ẵ|e|è|é|ẹ|ẻ|ẽ|ê|ề|ế|ệ|ể|ễ|i|ì|í|ị|ỉ|ĩ|o|ò|ó|ọ|ỏ|õ|ô|ồ|ố|ộ|ổ|ỗ|ơ|ờ|ớ|ợ|ở|ỡ|u|ù|ú|ụ|ủ|ũ|ư|ừ|ứ|ự|ử|ữ|y|ỳ|ý|ỵ|ỷ|ỹ|A|À|Á|Ạ|Ả|Ã|Â|Ầ|Ấ|Ậ|Ẩ|Ẫ|Ă|Ằ|Ắ|Ặ|Ẳ|Ẵ|E|È|É|Ẹ|Ẻ|Ẽ|Ê|Ề|Ế|Ệ|Ể|Ễ|I|Ì|Í|Ị|Ỉ|Ĩ|O|Ò|Ó|Ọ|Ỏ|Õ|Ô|Ồ|Ố|Ộ|Ổ|Ỗ|Ơ|Ờ|Ớ|Ợ|Ở|Ỡ|U|Ù|Ú|Ụ|Ủ|Ũ|Ư|Ừ|Ứ|Ự|Ử|Ữ|Y|Ỳ|Ý|Ỵ|Ỷ|Ỹ]{1})/g
    );
    if (isMatch != null) {
      word = "ngh" + word;
    }
    word = word.replace(
      /^([ngh]{3}|[ch|gh|gi|kh|ng|nh|ph|qu|th|tr]{2}|[b|c|d|đ|g|h|k|l|m|n|p|q|r|s|t|v|x]{1})/g,
      "nh"
    );
    textArr[i] = word;
  }
...
jQuery(document).ready(function () {
  ...
  jQuery("#btnSave").click(function () {
    ...
    jQuery("canvas").attr(
      "height",
      jQuery(".note-container").height() + 40 + "px"
    );
...
if (Theme.matches) {
var parent = document.getElementById("pcontent");
parent.classList.add("dark");
$('textarea').addClass('textarea-dark');
$('.btn-pcopy').addClass('btn-pcopy-dark');
$('.ptextarea').addClass('ptextarea-dark');
